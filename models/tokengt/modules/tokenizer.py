...
class GraphFeatureTokenizer(nn.Module):
    ...
    def get_batch_temporal(node_feature, edge_index, edge_feature, node_num, edge_num, perturb=None):
        """
        :param node_feature: Tensor([sum(node_num), D])
        :param edge_index: LongTensor([2, sum(edge_num)])
        :param edge_feature: Tensor([sum(edge_num), D])
    ...
    def forward(self, batched_data, perturb=None):
        ...
        # edge_index = torch.cat([idx for idx in edge_index])
        edge_index = torch.cat([idx.t() for idx in edge_index]).t()
        # lap_eigvec = torch.cat(lap_eigvec)
        # lap_eigvec = torch.cat([idx for idx in lap_eigvec])
        lap_eigvec = torch.cat([F.pad(i, (0, max(node_num) - i.size(1)), value=float('0')) for i in lap_eigvec])
        ##################################

        ## look-up table embedding
        # node_feature = self.node_encoder(node_data) # [batch_size * 2 , dim] = [64, 172]
        # edge_feature = self.node_encoder(edge_data)
        
        node_feature = node_feature
        edge_feature = edge_feature
        time_feature = time_feature
        # edge_feature_time = edge_feature + time_stamp_feature
        # edge_index = torch.cat([idx for idx in edge_index]).t()
        # edge_index = torch.cat([idx.t() for idx in edge_index]).t()
        device = edge_feature.device
        dtype = edge_feature.dtype
        
        # node_feature = torch.zeros((len(node_data), self.encoder_embed_dim))
