...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_embed_dim = hidden_dim # # hidden_dim = 200
        ...
        self.time_stamp_encoder = nn.Embedding(num_edges, hidden_dim, padding_idx=0)
    ...
    @staticmethod
    def get_batch(node_feature, edge_index, edge_feature, node_num, edge_num, time_stamp_feature, time_stamp_num, perturb=None):
        """
        ...
        """
        seq_len = [n + e + t for n, e, t in zip(node_num, edge_num, time_stamp_num)]
        b = len(seq_len)
        ...
        time_stamp_num = torch.tensor(time_stamp_num, device=device, dtype=torch.long)[:, None]  # [B, 1]
        ...
        # )
        # padded_time_stamp_mask = torch.logical_and(
        ...
        # # padded_index[padded_time_stamp_mask, :] = edge_index.t()
        
        # # padded_index = torch.zeros(b, max_len, 3, device=device, dtype=torch.long)  # [B, T, 3]
        # # padded_index[padded_node_mask, :2] = node_index.t()
        # # padded_index[padded_edge_mask, :2] = edge_index.t()
        # # padded_index[padded_time_stamp_mask, 2] = edge_index.t()
        
        # padded_index = torch.zeros(b, max_len, 3, device=device, dtype=torch.long)  # [B, T, 3]
        # # 형상 변경
        # padded_node_mask_expanded = padded_node_mask.unsqueeze(-1).expand_as(padded_index)

        # # 인덱싱
        # padded_index[padded_node_mask_expanded] = node_index.t().unsqueeze(0).expand_as(padded_index)
        ...
        # 노드 정보 저장
        # 노드 인덱스 재구성
        reshaped_node_index = node_index.t().unsqueeze(0).expand(b, max_n, 2)
        valid_node_mask = padded_node_mask.unsqueeze(-1).expand_as(reshaped_node_index)

        # 노드 정보 저장
        padded_index[valid_node_mask] = reshaped_node_index[valid_node_mask]


        # 엣지 정보 저장
        reshaped_edge_index = edge_index.t().unsqueeze(0).expand(b, -1, 2)
        padded_index[padded_edge_mask.unsqueeze(-1).expand_as(reshaped_edge_index), :2] = reshaped_edge_index

        # 타임스탬프 정보 저장
        # 여기서는 edge_index를 사용하였으나, 실제로는 타임스탬프에 해당하는 정보를 사용해야 합니다.
        reshaped_time_stamp = edge_index.t().unsqueeze(0).expand(b, -1, 2)
        padded_index[padded_time_stamp_mask.unsqueeze(-1).expand_as(reshaped_time_stamp), :2] = reshaped_time_stamp
    ...
    def forward(self, batched_data, perturb=None):
        (
            ...
            batched_data["time_stamp_data"],
            batched_data["time_stamp_num"],
        )
        ######### 전처리 수정핗요 #########
        node_data = torch.cat(node_data)
        edge_data = torch.cat(edge_data)
        time_stamp_data = torch.cat(time_stamp_data)
        
        node_num = torch.tensor(node_num).squeeze(1)
        edge_num = torch.tensor(edge_num).squeeze(1)
        time_stamp_num = torch.tensor(time_stamp_num).squeeze(1)
        
        # edge_index = torch.cat([idx.t() for idx in edge_index]).t()
        edge_index = torch.cat([idx for idx in edge_index]).t()
        lap_eigvec = torch.cat(lap_eigvec)
        ##################################
        
        node_feature = self.node_encoder(node_data)
        edge_feature = self.edge_encoder(edge_data)
        time_stamp_feature = self.time_stamp_encoder(time_stamp_data)

        device = node_feature.device
        dtype = node_feature.dtype

        padded_index, padded_feature, padding_mask, _, _ = self.get_batch(
            node_feature, edge_index, edge_feature, node_num, edge_num, time_stamp_feature, time_stamp_num, perturb
        )
