...
class DyGFormer(nn.Module):
    ...
    def pad_sequences(self, node_ids: np.ndarray, node_interact_times: np.ndarray, nodes_neighbor_ids_list: list, nodes_edge_ids_list: list,
        ...
        padded_nodes_neighbor_ids = np.zeros((len(node_ids), max_seq_length)).astype(np.float32)
        padded_nodes_edge_ids = np.zeros((len(node_ids), max_seq_length)).astype(np.float32)
