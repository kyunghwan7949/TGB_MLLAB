...
class TimeEncoder(nn.Module):
    ...
    #     emb = math.log(10000) / (half_dim - 1) # 로그 스케일 간격 조정
    #     emb = torch.exp(torch.arange(half_dim, dtype=torch.float) * -emb)
    #     emb = timestamps.type(dtype=torch.float)[:, None] * emb[None, :] # tiem encoding
...
#         timestamps = timestamps.unsqueeze(dim=2)
#         ts_cos = torch.cos(self.w(timestamps))
#         ts_sin = torch.sin(self.w(timestamps))
...
#         # output = torch.cat([ts_cos, ts_sin], dim=-2).flatten(-2, -1) * (self.time_dim ** -0.5)
#         cos_basis, sin_basis = ts_cos.unsqueeze(-2), ts_sin.unsqueeze(-2)    # [N, M*m, 1, time_dim], [N, M*m, 1, time_dim]
#         harmonic = torch.cat([cos_basis, sin_basis], dim=-2).flatten(-2, -1) * (self.time_dim ** -0.5)
#         return harmonic
#         # return output
    
class TimeEncode_Sin_Cos(nn.Module):
...
class TimeEncode_Sin_Cos(nn.Module):
    def __init__(self, time_dim: int, parameter_requires_grad: bool = True):
        super(TimeEncode_Sin_Cos, self).__init__()
    ...
    def forward(self, timestamps: torch.Tensor):
        ...
        # ts_cos, ts_sin = ts * self.basis_freq.view(1, 1, -1), ts * self.basis_freq.view(1, 1, -1)
        # cos_basis, sin_basis = torch.cos(ts_cos).unsqueeze(-2), torch.sin(ts_cos).unsqueeze(-2)    # [N, M*m, 1, time_dim], [N, M*m, 1, time_dim]
        harmonic = torch.cat([ts_cos, ts_sin], dim=-2).flatten(-2, -1) * (self.time_dim ** -0.5)
        return harmonic
