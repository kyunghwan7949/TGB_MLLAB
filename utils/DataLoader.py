...
def get_link_pred_data_TRANS_TGB(dataset_name: str):
    ...
    edge_raw_features =  data.msg.numpy() # Messages feature matrix with shape : [num_events, num_msg_features] = [157474, 172]
    node_raw_features = np.zeros((data.dst.size(0), NODE_FEAT_DIM)) # [num_events, num_msg_features]
    ...
    src_node_ids = (data.src.numpy()+1).astype(np.longlong) # (num_events, )
    dst_node_ids = (data.dst.numpy()+1).astype(np.longlong) # (num_events, )
    node_interact_times = data.t.numpy().astype(np.float64) # (num_events, )
    edge_ids = np.array([i for i in range(1, len(data.src)+1)]).astype(np.longlong) # (num_events, )
    labels = data.y.numpy() # (num_events, )
    ...
    return node_raw_features, edge_raw_features, \
           full_data, train_data, val_data, test_data, dataset, \
