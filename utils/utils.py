...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in tqdm(enumerate(adj_list), desc="Loading nodes..."):
    ...
    def compute_sampled_probabilities(self, node_neighbor_times: np.ndarray):
        """
        'time_interval_aware' strategy를 선택하는 경우 샘플링 확률 계산
    ...
    def get_directly_connected_neighbors(self, node_id: int, interact_time: float):
        """
        해당 노드와 직접 연결된 모든 이웃 노드들을 반환합니다.
        :param node_id: int, 노드 ID
        :param interact_time: float, 상호작용 시간
        :return: (src_node_id, dst_node_id) 쌍의 리스트
        """
        # node_id와 연결된 모든 이웃 노드를 찾습니다.
        node_neighbor_ids, _, _, _ = self.find_neighbors_before(node_id=node_id, 
                                                               interact_time=interact_time,
                                                               return_sampled_probabilities=False)
        
        return [(node_id, dst_node_id) for dst_node_id in node_neighbor_ids]

    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        해당 노드에 대해 특정 시간 이전에 상호 작용한 neighbor node 찾기
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        """
        해당 노드에서 historical neighbor node 찾기
        ...
        # tmp = self.get_directly_connected_neighbors(node_id=node_id, interact_time=node_interact_time)
        ...
        for idx, (node_id, node_interact_time) in tqdm(enumerate(zip(node_ids, node_interact_times))):
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        """
        해당 노드와 직접 연결 된 first(one)-hop 모든 neighbor node 들을 sequence로 저장
    ...
    def get_all_first_hop_neighbors_tokengt(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        """
        해당 노드와 직접 연결 된 first(one)-hop 모든 neighbor node 들을 sequence로 저장
        get historical neighbors of nodes in node_ids at the first hop with max_num_neighbors as the maximal number of neighbors (make the computation feasible)
        :param node_ids: ndarray, shape (batch_size, ), node ids
        ...
        """
        # three lists to store the first-hop neighbor ids, edge ids and interaction timestamp information, with batch_size as the list length
        connected_node_pair = [] # 1-hop 연결 노드쌍
        edge_id_list = []
        ...
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            ...
            nodes_edge_ids_list.append(node_edge_ids)
            nodes_neighbor_times_list.append(node_neighbor_times)
            # tmp.append((node_ids, node_neighbor_ids))
            
            for idx, (neighbor_node_id, neighbor_edge_id) in enumerate(zip(node_neighbor_ids, node_edge_ids)):
                connected_node_pair.append((node_id, neighbor_node_id))
                edge_id_list.append(neighbor_edge_id)
                
        
        connected_node_pair = [pair for pair in connected_node_pair if pair != (8228,1)] 
        ## node
        reverse = connected_node_pair + [(y, x) for (x, y) in connected_node_pair]
        # reverse_set = list(set(reverse)) # 'time set'정보 고려 x
        
        ## edge
        node_id_pair = []
        node_id_index = []
        for idx, item in enumerate(connected_node_pair):
            if item not in node_id_pair:
                node_id_pair.append(item)
                node_id_index.append(idx)

        edge_index_reverse = [edge_id_list[idx] for idx in node_id_index] * 2
        node_id_pair_reverse = node_id_pair + [(y, x) for (x, y) in node_id_pair]

        return nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list, node_id_pair_reverse, edge_index_reverse
    
    
    def get_neg_stack_all_first_hop_neighbors_tokengt(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        """
        해당 노드와 직접 연결 된 first(one)-hop 모든 neighbor node 들을 sequence로 저장
        get historical neighbors of nodes in node_ids at the first hop with max_num_neighbors as the maximal number of neighbors (make the computation feasible)
        :param node_ids: ndarray, shape (batch_size, ), node ids
...
def get_neighbor_sampler(data: pd.DataFrame, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    # max_node_id = max(data.src_node_ids.max(), data.dst_node_ids.max())
    # graph_num_node = max_node_id+1
    # the adjacency vector stores edges for each node (source or destination), undirected
    # adj_list, list of list, where each element is a list of triple tuple (node_id, edge_id, timestamp)
    # the list at the first position in adj_list is empty
    ## 'dst_node_id'/ 'src_node_id' / 'time_stamp' 를 인접리스트에 추가 (모든 그래프 관계)


    # adj_list = [[] for _ in range(max_node_id + 1)]
    # for src_node_id, dst_node_id, edge_id, node_interact_time in tqdm(zip(data.src_node_ids, data.dst_node_ids, data.edge_ids, data.node_interact_times), ncols=120, desc="Loading edges..."):

    #     adj_list[src_node_id].append((dst_node_id, edge_id, node_interact_time))
    #     adj_list[dst_node_id].append((src_node_id, edge_id, node_interact_time))
    
    # # 각 노드 인접리스트 저장
    # with open('/data/data2/khahn/TGB/tgb/datasets/tgbl_wiki/adj_list_wiki.pkl', 'wb') as f:
    #     pickle.dump(adj_list, f)
    
    # with open("/data/data2/tgb/tgbl_flight/adj_list_flight.pkl","rb") as fr:
    #     adj_list = pickle.load(fr)
        
    # # 각 노드 인접리스트 저장
    # with open('/data/data2/khahn/TGB/tgb/datasets/tgbl_flight/adj_list_flight.pkl', 'wb') as f:
    #     pickle.dump(adj_list, f)
    
    # with open("/data/data2/khahn/TGB/tgb/datasets/tgbl_wiki/adj_list_wiki.pkl","rb") as fr:
    #     adj_list = pickle.load(fr)
    
    # sampler = NeighborSampler(adj_list=adj_list, sample_neighbor_strategy=sample_neighbor_strategy, time_scaling_factor=time_scaling_factor, seed=seed)
    # connected_neighbors = sampler.get_directly_connected_neighbors(node_id=some_node_id, interact_time=some_time)
...
class NegativeEdgeSampler(object):
    ...
    def get_unique_edges_between_start_end_time(self, start_time: float, end_time: float):
        """
        주어진 negative sampling method에 의해 negative edge sampling
    ...
    def random_sample(self, size: int):
        """
        random sampling : 그래프 내의 가능한 모든 노드 쌍 중에서 무작위로 negative edge를 샘플링
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        """
        historical sampling : 과거의 엣지를 기반으로 negative edge를 샘플링합니다. 현재 배치에 나타나지 않는 과거의 엣지를 negative sample로 선택
    ...
    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
                         current_batch_start_time: float, current_batch_end_time: float):
        """
        inductive sampling : 주어진 마지막 관찰 시간(last_observed_time) 이후의 엣지를 제외하고, 나머지 가능한 엣지들 중에서 negative edge를 샘플링 
