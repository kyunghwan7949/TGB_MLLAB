...
def load_edgelist_wiki(fname: str) -> pd.DataFrame:
    ...
    tmp = pd.DataFrame({"u": src, "i": dst, "ts": t, "idx": idx, "w": w}), msg, None
...
def load_edgelist_sr(
    ...
    #print("number of lines counted", num_lines)
    print("there are ", num_lines, " lines in the raw data")
    u_list = np.zeros(num_lines) # user_id
    i_list = np.zeros(num_lines) # item_id
    ts_list = np.zeros(num_lines) # timestamp
    label_list = np.zeros(num_lines)
    feat_l = np.zeros((num_lines, feat_size)) # comma_separated_list_of_features
    idx_list = np.zeros(num_lines)
    w_list = np.zeros(num_lines)
...
def csv_to_pd_data(
    ...
    with open(fname, "r") as csv_file:
        ...
        for row in tqdm(csv_reader):
            if idx == 0:
                ...
                ts = row[0]
                date_cur = datetime.strptime(ts, TIME_FORMAT) # 'day'를 timestamp로 변환
    ...
    return (
        ...
        pd.DataFrame(
            {
                "u": u_list, # source airpot
                "i": i_list, # destination airport
                "ts": ts_list, # timestamp
                "label": label_list, 
                "idx": idx_list,
                "w": w_list, # weight
            }
